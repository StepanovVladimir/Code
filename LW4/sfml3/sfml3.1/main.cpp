...
void update(const sf::Vector2f &mousePosition, sf::ConvexShape &pointer, float &dt)
{
    ...
    const float timeRotation = 15 * dt;
    const float mouseRotation = std::abs((angle - pointer.getRotation()));
    if (angle > pointer.getRotation())
    {
        if ((angle - 180) < pointer.getRotation())
        {
            pointer.setRotation(pointer.getRotation() + std::min(timeRotation, mouseRotation));
        }
        else
        {
            pointer.setRotation(pointer.getRotation() - std::min(timeRotation, mouseRotation));
...
int main()
{
    ...
    sf::RenderWindow window(
        ...
        "Prints follows mouse", sf::Style::Default, settings);
    sf::Clock clock;
